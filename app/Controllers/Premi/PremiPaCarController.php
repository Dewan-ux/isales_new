...
use App\Models\ProdukbModel;
use App\Models\PremibModel;

class PremiPaCarController extends BaseController
{
    public function __construct()
    {
        $this->auth = new LoginModel();
        $this->d_premi = new PremibModel();
        $this->d_produk = new ProdukbModel();
    }
    ...
    public function create()
    {
        ...
        if(!$req)
        {
            ...
            if(!$val = tokenCheck($req))
            {
                ...
                {
                     if($this->validate->run($req, 'tambahPremi') === FALSE)
                    {
                        $res = [
                            'status' => 400,
                            'error' => true,
                        ...
                        $t_premi_data = [
                            'nominal'    => $req['nominal'],
                            'satuan'     => $req['satuan'],
                            'id_produk_pa_car'  => $req['id_produk_pa_car'],
                            'up'         => $req['up'],
                            'manfaat'         => $req['manfaat'],
                            'created_by' => $val['id']
                        ];
    ...
    public function updatePremi()
    {
        ...
        if(!$req)
        {
            ...
            if(!$val = tokenCheck($req))
            {
                ...
                if($val['role'] == '1' || $val['role'] == '5')
                {
                     if($this->validate->run($req, 'ubahPremi') === FALSE)
                    {
                        $res = [
                            'status' => 400,
                            'error' => true,
