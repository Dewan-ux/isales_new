...
class PremiPaModel extends Model
{
    protected $table = 't_premi_pa_car';
	protected $primaryKey = 'id';

	protected $allowedFields = ['nominal', 'id_produk', 'satuan', 'kategori','aktif','up','created_by','created_at'];
	...
	public function getAll($param = array())
	{
		...
		if (isset($param['id_produk'])) { $this->where('t_premi_pa_car.id_produk_pa_car', $param['id_produk']); }
		...
		if(isset($param['list']))
		{
			if($param['list'] == 1){
				...
				$this->select('t_produk_pa_car.id as id_produk, t_produk_pa_car.nama_produk, t_premi_pa_car.id as id, t_premi_pa_car.nominal, t_premi_pa_car.satuan, t_premi_pa_car.up');
				$this->join('t_produk_pa_car', 't_produk_pa_car.id = t_premi_pa_car.id_produk_pa_car');
				// $this->where('t_premi_pa_car.satuan', $param['satuan']);
